...
\setbeamertemplate{title page}{%
  ...
  \parbox[t]{0.85\paperwidth}{%
    ...
    \vspace{0.10cm}
    {\color{white!55}\footnotesize\insertinstitute%
      \enspace\textbar\enspace\insertdate\par}%
  }%
}
...
% =============================================================================
\begin{frame}{Functional Verification}
% =============================================================================
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      ...
      \vspace{0.35cm}
      \textbf{Results} \quad (tol $= 10^{-4}$)
      \vspace{0.1cm}

      {\small
      \begin{tabular}{cccll}
        \toprule
        ID & $N$ & $d$ & Causal & Max $|$err$|$ \\
        \midrule
        ...
        3 & 16 & 8 & No  & $1.2\times10^{-7}$\quad\good{\checkmark} \\
        4 & 16 & 8 & Yes & $1.6\times10^{-7}$\quad\good{\checkmark} \\
        \bottomrule
      \end{tabular}
      }
    \end{column}
    \begin{column}{0.40\textwidth}
      \begin{block}{\good{5 / 5 PASS}}
        Max error: $\mathbf{1.6\times10^{-7}}$\\
        \muted{$600\times$ below tolerance.}
      \end{block}
      ...
      \hrulefull
      \begin{itemize}\setlength\itemsep{3pt}
        \item Single-tile path ($N \le 8$)
        \item \hl{Multi-tile path} ($N{=}16$:\\
              2 Q-tiles $\times$ 2 K-tiles)
        \item Full and causal masking
        \item Non-multiple-of-tile $N$
        \item C-sim = RTL co-sim output
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
% =============================================================================
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{\good{Delivered}}
        ...
        \begin{itemize}\setlength\itemsep{3pt}
          ...
          \item \good{5/5 tests pass} in C-sim and RTL co-sim
          \item Max error: $1.6\times10^{-7}$
                ($600\times$ below tolerance)
    ...
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{\muted{Limitations}}
        \vspace{2pt}
        \begin{itemize}\setlength\itemsep{2pt}
          \item Timing violation ($-1.40$ ns at 100 MHz)
          \item DSPs at 81\% limit D\_MAX to 32
          \item Single head; sequential tile execution
        \end{itemize}
        \vspace{2pt}
      \end{block}
