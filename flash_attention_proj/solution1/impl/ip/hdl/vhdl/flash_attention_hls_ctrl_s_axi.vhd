...
entity flash_attention_hls_ctrl_s_axi is
generic (
...
end entity flash_attention_hls_ctrl_s_axi;
...
architecture behave of flash_attention_hls_ctrl_s_axi is
    ...
    constant ADDR_D_DATA_0      : INTEGER := 16#48#;
    constant ADDR_D_CTRL        : INTEGER := 16#4c#;
    constant ADDR_CAUSAL_DATA_0 : INTEGER := 16#50#;
    constant ADDR_CAUSAL_CTRL   : INTEGER := 16#54#;
    ...
    signal int_causal          : UNSIGNED(31 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CAUSAL_DATA_0 =>
                        rdata_data <= RESIZE(int_causal(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CAUSAL_DATA_0) then
                    int_causal(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_causal(31 downto 0));
                end if;
            end if;
        end if;
    end process;
